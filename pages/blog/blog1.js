...
const blog1 = [
  ...
  const filteredblog1 =
    activeCategory === "All"
      ? blog1
      : blog1.filter((blog1) =>
          blog1.categories.includes(activeCategory)
        );
...
<section className="relative min-h-screen flex items-center px-6 text-white bg-black overflow-hidden">
  ...
  <div className="relative z-20 max-w-5xl ml-8">
    <h1 className="text-5xl sm:text-6xl font-bold uppercase leading-tight mb-4 animate-fade">
       {translations.heroBlog1?.title || "Loading..."}
    </h1>
  ...
  / <span className="font-semibold text-[#58a3ff]">{translations.heroBlog1?.breadcrumb}</span>
</p>
  </div>
</section>
...
<section className="py-10 px-6 bg-white">
  <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-[250px_1fr] gap-10">
    ...
    <div className="space-y-10">
      {filteredblog1.map((item, index) => {
        const blog1 = translations.blog1?.[item.key];
        const meta = translations.blog1Meta || {};

        if (!blog1) return null;
...
<span className="flex items-center gap-1">
  ...
  <span className="flex items-center gap-1">
    ...
    <span className="ml-1">14719</span>
  </span>
</div>
            ...
            <div className="space-y-4 text-[#4F4F4F] text-base leading-relaxed">
              {blog1.title1 && (
                <>
                  <h4 className="font-semibold text-black mb-1">{blog1.title1}</h4>
                  {blog1.description1 && <p className="mt-0">{blog1.description1}</p>}
                </>
              )}
              {blog1.description1_2 && <p>{blog1.description1_2}</p>}

              {blog1.title2 && (
                <>
                  <h4 className="font-semibold text-black mb-1">{blog1.title2}</h4>
                  {blog1.description2 && <p className="mt-0">{blog1.description2}</p>}
                </>
              )}
              {blog1.description2_2 && <p>{blog1.description2_2}</p>}
              {blog1.description2_3 && <p className="italic">{blog1.description2_3}</p>}
              {blog1.description2_4 && <p>{blog1.description2_4}</p>}

              {blog1.title3 && (
                <>
                  <h4 className="font-semibold text-black mb-1">{blog1.title3}</h4>
                  {blog1.description3 && <p className="mt-0">{blog1.description3}</p>}
                </>
              )}

              {blog1.title4 && (
                <>
                  <h4 className="font-semibold text-black mb-1">{blog1.title4}</h4>
                  {blog1.description4 && <p className="mt-0">{blog1.description4}</p>}
                </>
              )}
              {blog1.description4_2 && <p className="italic">{blog1.description4_2}</p>}
              {blog1.by && <p className="italic">{blog1.by}</p>}
              {blog1.source && <p>{blog1.source}</p>}
            </div>
...
<div className="grid grid-cols-1 md:grid-cols-2 gap-10 mt-12 border-t border-gray-200 pt-8">
  ...
  <div>
    <h3 className="text-lg font-black uppercase mb-5 tracking-tight">
      {translations.related?.categoryTitle || "In Same Category"}
    </h3>
    <ul className="space-y-4 text-sm text-black">
      {[
        { title: translations.related?.albania, href: "/blog/blog2" },
        { title: translations.related?.business, href: "/blog/blog5" },
        { title: translations.related?.when, href: "/blog/blog4" },
        { title: translations.related?.the, href: "/blog/blog3" },
      ].map((item, index) => (
  ...
  <div>
    <h3 className="text-lg font-black uppercase mb-5 tracking-tight">
      {translations.related?.tagsTitle || "Related by Tags"}
    </h3>
    <ul className="space-y-4 text-sm text-black">
      {[
        { title: translations.related?.do, href: "/blog/blog1" },
        { title: translations.related?.albaniaover, href: "/blog/blog2" },
      ].map((item, index) => (
        <li key={index} className="border-b border-dotted border-gray-300 pb-2">
          <a href={item.href} className="hover:underline">
            {item.title}
...
<div className="mt-12 max-w-2xl">
  ...
  <form className="space-y-6 text-sm text-black">
    ...
    <div className="flex flex-col md:flex-row md:items-center gap-2">
      ...
      <div className="flex items-center gap-2">
        <img
          src="/img/captcha.jpg" // Replace with actual captcha image source
          alt="captcha"
