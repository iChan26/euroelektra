...
const blog3 = [
  ...
  const filteredblog3 =
    activeCategory === "All"
      ? blog3
      : blog3.filter((blog3) =>
          blog3.categories.includes(activeCategory)
        );
...
<section className="relative min-h-screen flex items-center px-6 text-white bg-black overflow-hidden">
  ...
  <div className="relative z-20 max-w-5xl ml-8">
    <h1 className="text-5xl sm:text-6xl font-bold uppercase leading-tight mb-4">
      {translations.heroBlog3?.title || "Loading..."}
    </h1>
    <div className="w-24 h-1 bg-white mb-4" />
   <p className="flex items-center gap-2">
  <FaHome className="text-[#ffffff]" />
  / <span className="font-semibold text-[#58a3ff]">{translations.heroBlog3?.breadcrumb || "Loading..."}</span>
</p>
  </div>
</section>
...
<section className="py-10 px-6 bg-white">
  <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-[250px_1fr] gap-10">
    ...
    <div className="space-y-10">
      {filteredblog3.map((item, index) => {
        const blog3 = translations.blog3?.[item.key];
        const meta = translations.blog1Meta || {};

        if (!blog3) return null;
...
<div className="flex flex-wrap items-center text-sm text-[#333] gap-x-5 gap-y-3 mb-4">
 ...
 <span className="flex items-center gap-1">
  ...
  <span className="flex items-center gap-1">
    ...
    <span className="ml-1">10326</span>
  </span>
</div>
...
{blog3?.title1 && (
  <>
    <h4 className="font-semibold text-black mb-1">{blog3.title}</h4>
    
  </>
)}

{/* Paragraphs Section */}
{blog3.description1 && <p>{blog3.description1}</p>}
{blog3.description1_2 && <p>{blog3.description1_2}</p>}
{blog3.description1_3 && <p>{blog3.description1_3}</p>}
{blog3.description1_4 && <p className="italic">{blog3.description1_4}</p>}
...
<div className="grid grid-cols-1 md:grid-cols-2 gap-10 mt-12 border-t border-gray-200 pt-8">
  ...
  <div>
    <h3 className="text-lg font-black uppercase mb-5 tracking-tight">
      {translations.related3?.categoryTitle || "IN SAME CATEGORY"}
    </h3>
    <ul className="space-y-4 text-sm text-black">
      {[
        { key: 'do', href: '/blog/blog1' },
        { key: 'albania', href: '/blog/blog2' },
        { key: 'business', href: '/blog/blog5' },
        { key: 'when', href: '/blog/blog4' }
      ].map(({ key, href }, index) => (
        <li
          key={index}
  ...
  <div>
    <h3 className="text-lg font-black uppercase mb-5 tracking-tight">
      {translations.related3?.tagsTitle || "RELATED BY TAGS"}
    </h3>
...
<div className="mt-12 max-w-2xl">
  ...
  <form className="space-y-6 text-sm text-black">
    ...
    <div className="flex flex-col md:flex-row md:items-center gap-2">
      ...
      <div className="flex items-center gap-2">
        <img
          src="/img/captcha3.jpg" // Replace with actual captcha image source
          alt="captcha"
