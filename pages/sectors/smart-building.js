...
<section className="relative min-h-screen flex items-center justify-center px-6 text-white bg-black overflow-hidden">
  ...
  <div className="relative z-20 max-w-4xl text-center">
    <h1 className="text-5xl sm:text-6xl font-extrabold uppercase mb-8">
      {translations.smartBuildingHero?.title}
    </h1>
    <p className="text-lg sm:text-xl font-medium text-white max-w-3xl mx-auto">
      {translations.smartBuildingHero?.description}
    </p>
  </div>
</section>
...
<section className="bg-white px-6 py-16 border-t border-gray-200">
  <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-[250px_1fr] gap-10 items-start">
       ...
       {/* 🟦 SIDEBAR */}
    <aside className="w-full md:w-[250px] max-h-[700px] overflow-y-auto pr-2">
      <h2 className="text-lg font-semibold text-[#1C1C1C] mb-4">
        {translations.smartBuildingHero?.title || "Electrical Products"}
      </h2>
    ...
    <div className="space-y-10">
      {Array.isArray(translations?.sections) &&
        translations.sections.map((section, idx) => (
          <div
            key={idx}
            className={`bg-[#F5F7FA] p-6 rounded-md shadow-sm border border-gray-200 flex flex-col md:flex-row gap-6 ${
            ...
            <div className="flex-1 space-y-4">
              ...
              {Array.isArray(section.paragraphs) &&
                section.paragraphs.map((para, i) => (
                  <p key={i} className="text-[#4F4F4F] text-sm leading-relaxed">
                    {para}
                  </p>
              ))}
            </div>
